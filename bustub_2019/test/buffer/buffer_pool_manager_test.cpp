...
TEST(BufferPoolManagerTest, SampleTest) {
  ...
  for (int i = 0; i < 4; ++i) {
    EXPECT_NE(nullptr, bpm->NewPage(&page_id_temp));
    EXPECT_EQ(i+10, page_id_temp); //Add by sparkyen
  }

  // Scenario: We should be able to fetch the data we wrote a while ago.
  page0 = bpm->FetchPage(0);
  EXPECT_EQ(page0->GetPageId(), 0); //Add by sparkyen
  EXPECT_EQ(0, strcmp(page0->GetData(), "Hello"));
...
TEST(BufferPoolManagerTest, IsDirty) {
  ...
  EXPECT_EQ(1, page->GetPinCount()); //Add by sparkyen
  ...
  page = bpm->FetchPage(0);
  ASSERT_NE(nullptr, page);
  EXPECT_EQ(0, page->GetPageId()); //Add by sparkyen
  EXPECT_FALSE(page->IsDirty());
  EXPECT_EQ(0, strcmp(page->GetData(), "Database"));
